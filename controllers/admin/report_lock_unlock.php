...
if ($is_lock == 1) {
    ...
    WHERE transaction_id = ?;")) {
        ...
        echo "Query Error (report_lock_unlock.php stmt1->prepare) : $stmt1->errno : $stmt1->error";
        $response_msg = "failed";
    }
    if ($response_msg != "failed") {
        notif_insert(2, $transaction_id, $admin_id);
    }
} else {
    if ($stmt2 -> prepare("
    UPDATE `assistance_transactions`
    ...
    WHERE transaction_id = ?;")) {
        ...
        echo "Query Error (report_lock_unlock.php stmt2->prepare) : $stmt2->errno : $stmt2->error";
        $response_msg = "failed";
    }
    if ($response_msg != "failed") {
        notif_insert(3, $transaction_id, $admin_id);
    }
}
