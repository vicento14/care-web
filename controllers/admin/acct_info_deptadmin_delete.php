...
if ($stmt1 -> prepare(
  ...
  echo "Query Error (acct_info_deptadmin_delete.php stmt1->prepare) : $stmt1->errno : $stmt1->error";
}
...
WHERE account_id = ?;")) {
  ...
  echo "Query Error (acct_info_deptadmin_delete.php stmt2->prepare) : $stmt2->errno : $stmt2->error";
}
...
if ($stmt3 -> prepare("DELETE FROM `account_roles` WHERE account_role_id = ?;")) {
  ...
  echo "Query Error (acct_info_deptadmin_delete.php stmt3->prepare) : $stmt3->errno : $stmt3->error";
}
...
if ($stmt4 -> prepare("DELETE FROM `account_auths` WHERE account_auth_id = ?;")) {
  ...
  echo "Query Error (acct_info_deptadmin_delete.php stmt4->prepare) : $stmt4->errno : $stmt4->error";
}
