...
<title>CARE | Concern Details</title>

<div class="container-fluid">

    <div class="d-sm-flex align-items-center justify-content-end mb-4">
        <button type="button" class="d-none d-sm-inline-block btn btn-sm btn-success shadow-sm" id="backReport"><i class="fas fa-arrow-left"></i> Back to Concern Report</button>
    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-stretch">
                        <form action="#lock" method="post" class="form">
                            <input type="button" class="btn btn-success  ml-5 mb-4" id="lock_unlock_report_concerns" value="Lock">
                        </form>

                        <form action="#Mark" method="post" class="form">
                            <input type="button" class="btn btn-success  ml-5 mb-4" id="resolve_report_concerns" value="Mark as Resolved">
                        </form>
                        ...
                        <form action="#Back" method="post" class="form">
                            <input type="button" class="btn btn-success  ml-5 mb-4" id="back_report_concerns" value="Back">
                        </form>
                    </div>
...
<script>
    const lock_unlock_report_concerns = document.querySelector('#lock_unlock_report_concerns');
    const resolve_report_concerns = document.querySelector('#resolve_report_concerns');
    const back_report_concerns = document.querySelector('#back_report_concerns');
    ...
    const sendData = (data, url) => {
        if (url == "../controllers/admin/report_concerns_lock_unlock.php") {
        ...
        } else if (url == "../controllers/admin/report_concerns_resolve.php") {
            document.querySelector('#LoadModalTitle').innerHTML = "Reports";
        ...
        sendHttpRequest('POST', url, data).then(responseData => {
            if(url == "../controllers/admin/report_concerns_view.php"){
                ...
                document.getElementById("msg_user_account_name").innerHTML = transaction_array.user_account_name;
                if (transaction_array.is_resolved == "pending") {
                    if (transaction_array.is_lock == 1) {
                        document.getElementById("resolve_report_concerns").disabled = false;
                        document.getElementById("lock_unlock_report_concerns").value = "Unlock";
                        document.getElementById("lock_unlock_report_concerns").disabled = false;
                        ...
                        document.getElementById("resolve_report_concerns").disabled = true;
                        document.getElementById("lock_unlock_report_concerns").value = "Lock";
                        document.getElementById("lock_unlock_report_concerns").disabled = false;
                    ...
                    document.getElementById("resolve_report_concerns").disabled = true;
                    document.getElementById("lock_unlock_report_concerns").value = "Lock";
                    document.getElementById("lock_unlock_report_concerns").disabled = true;
                    document.getElementById("report_response_details").disabled = true;
                    document.getElementById("insert_report_response").disabled = true;
                }
            ...
            } else if (url == "../controllers/admin/report_concerns_lock_unlock.php") {
                let response_array = JSON.parse(responseData);
                if (response_array.response_msg == "success" && response_array.is_lock == 1) {
                    document.querySelector("#lock_unlock_report_concerns").value = "Unlock";
                    document.getElementById("resolve_report_concerns").disabled = false;
                    document.getElementById("report_response_details").disabled = false;
                    document.getElementById("insert_report_response").disabled = false;
                    setTimeout(function () {
                    ...
                    document.querySelector("#lock_unlock_report_concerns").value = "Lock";
                    document.getElementById("resolve_report_concerns").disabled = true;
                    document.getElementById("report_response_details").disabled = true;
                    document.getElementById("insert_report_response").disabled = true;
                    setTimeout(function () {
    ...
    const refresh_report = () => {
        ...
        if (transaction_id != ''){
            sendData(data, "../controllers/admin/report_concerns_view.php");
            sendData(data, "../controllers/admin/report_response_view.php");
    ...
    lock_unlock_report_concerns.addEventListener('click', () => {
        let transaction_id = document.querySelector("#transaction_id").value;
        let is_lock;
        if (document.querySelector("#lock_unlock_report_concerns").value === "Lock") {
            is_lock = 1;
            let data = `transaction_id=${transaction_id}&is_lock=${is_lock}`;
            if (transaction_id != ''){
                sendData(data, "../controllers/admin/report_concerns_lock_unlock.php")
            } else {
                alert("Internal Error! the transaction id was not fetched");
            }
        } else {
    ...
    resolve_report_concerns.addEventListener('click', () => {
        let transaction_id = document.querySelector("#transaction_id").value;
        let data = `transaction_id=${transaction_id}`;
        if (transaction_id != ''){
            sendData(data, "../controllers/admin/report_concerns_resolve.php");
        } else {
            alert("Internal Error! the transaction id was not fetched");
        }
    })
    ...
    back_report_concerns.addEventListener('click', () => {
        window.localStorage.removeItem("transaction_id");
        window.location.href = "../admin/report_concerns.php";
    })
