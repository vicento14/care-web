...
<title>CARE | Report Details</title>

<div class="container-fluid">

    <!-- Transfer Report Modal -->
    <div class="modal fade" id="transferReportModal" tabindex="-1" role="dialog" aria-labelledby="transferReportModalTitle" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="transferReportModalTitle">Transfer Report</h5>
                ...
                <div class="modal-body">
                    ...
                    <p>Are you sure you want to transfer this report?</p>
                    <p class="text-warning"><small>This action cannot be undone.</small></p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">No</button>
                    <button type="button" class="btn btn-danger" id="confirm_transfer_report">Yes</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="d-sm-flex align-items-center justify-content-end mb-4">
        <button type="button" class="d-none d-sm-inline-block btn btn-sm btn-success shadow-sm" id="backReport"><i class="fas fa-arrow-left"></i> Back to Reports</button>
    </div>
                ...
                <div class="card-body">
                    <div class="row no-gutters align-items-between">
                        ...
                        <div class="col mr-2">
                            ...
                            <p>Report Type : <span id="report_type_name" > 0000000000000</span></p>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-stretch">
                        <form action="#lock" method="post" class="form">
                            <input type="button" class="btn btn-success  ml-5 mb-4" id="lock_unlock_reports" value="Lock">
                        </form>

                        <form action="#Mark" method="post" class="form">
                            <input type="button" class="btn btn-success  ml-5 mb-4" id="resolve_reports" value="Mark as Resolved">
                        </form>

                        <form action="#Transfer" method="post" class="form">
                            <input type="button" class="btn btn-success  ml-5 mb-4" id="transfer_reports" value="Transfer Report" href="#" data-toggle="modal" data-target="#transferReportModal">
                        </form>
                        ...
                        <form action="#Back" method="post" class="form">
                            <input type="button" class="btn btn-success  ml-5 mb-4" id="back_reports" value="Back">
                        </form>
                    </div>
...
<script>
    ...
    const lock_unlock_reports = document.querySelector('#lock_unlock_reports');
    const resolve_reports = document.querySelector('#resolve_reports');
    const back_reports = document.querySelector('#back_reports');
    const insert_report_response = document.querySelector('#insert_report_response');
    const confirm_transfer_report = document.querySelector('#confirm_transfer_report');
    // HttpRequest Data
    ...
    const sendData = (data, url) => {
        if (url == "../controllers/admin/report_lock_unlock.php") {
        ...
        } else if (url == "../controllers/admin/report_resolve.php") {
            document.querySelector('#LoadModalTitle').innerHTML = "Reports";
            document.querySelector('#LoadModalBody').innerHTML = "Updating Report Status. Please wait...";
            $('#LoadModal').modal('show');
        } else if (url == "../controllers/admin/report_transfer.php") {
            $('#transferReportModal').modal('hide');
            document.querySelector('#LoadModalTitle').innerHTML = "Reports";
            document.querySelector('#LoadModalBody').innerHTML = "Transferring Report to another department. Please wait...";
            $('#LoadModal').modal('show');
        }
        sendHttpRequest('POST', url, data).then(responseData => {
            if(url == "../controllers/admin/report_view.php"){
                ...
                document.getElementById("report_type_name").innerHTML = transaction_array.report_type_name;
                ...
                if (transaction_array.is_resolved == "pending") {
                    if (transaction_array.is_lock == 1) {
                        document.getElementById("transfer_reports").disabled = true;
                        document.getElementById("resolve_reports").disabled = false;
                        document.getElementById("lock_unlock_reports").value = "Unlock";
                        document.getElementById("lock_unlock_reports").disabled = false;
                        ...
                        document.getElementById("transfer_reports").disabled = false;
                        document.getElementById("resolve_reports").disabled = true;
                        document.getElementById("lock_unlock_reports").value = "Lock";
                        document.getElementById("lock_unlock_reports").disabled = false;
                    ...
                    document.getElementById("resolve_reports").disabled = true;
                    document.getElementById("lock_unlock_reports").value = "Lock";
                    document.getElementById("lock_unlock_reports").disabled = true;
                    document.getElementById("report_response_details").disabled = true;
                    document.getElementById("insert_report_response").disabled = true;
                }
            ...
            } else if (url == "../controllers/admin/report_lock_unlock.php") {
                let response_array = JSON.parse(responseData);
                if (response_array.response_msg == "success" && response_array.is_lock == 1) {
                    document.querySelector("#lock_unlock_reports").value = "Unlock";
                    document.getElementById("transfer_reports").disabled = true;
                    document.getElementById("resolve_reports").disabled = false;
                    document.getElementById("report_response_details").disabled = false;
                    document.getElementById("insert_report_response").disabled = false;
                    ...
                    document.querySelector("#lock_unlock_reports").value = "Lock";
                    document.getElementById("transfer_reports").disabled = false;
                    document.getElementById("resolve_reports").disabled = true;
                    document.getElementById("report_response_details").disabled = true;
                    document.getElementById("insert_report_response").disabled = true;
                ...
                if (responseData == "success") {
                    ...
                    setTimeout(function () {
                        $('#LoadModal').modal('hide');
                        document.querySelector('#MsgboxModalTitle').innerHTML = "Reports";
                        document.querySelector('#MsgboxModalBody').innerHTML = "Report Transferred Successfully";
                        $('#MsgboxModal').modal('show');
                    },500);
                } else {
                    setTimeout(function () {
    ...
    const refresh_report = () => {
        ...
        if (transaction_id != ''){
            sendData(data, "../controllers/admin/report_view.php");
            sendData(data, "../controllers/admin/report_response_view.php");
    ...
    lock_unlock_reports.addEventListener('click', () => {
        let transaction_id = document.querySelector("#transaction_id").value;
        let is_lock;
        if (document.querySelector("#lock_unlock_reports").value === "Lock") {
            is_lock = 1;
            let data = `transaction_id=${transaction_id}&is_lock=${is_lock}`;
            if (transaction_id != ''){
                sendData(data, "../controllers/admin/report_lock_unlock.php")
            } else {
                alert("Internal Error! the transaction id was not fetched");
            }
        } else {
    ...
    resolve_reports.addEventListener('click', () => {
        let transaction_id = document.querySelector("#transaction_id").value;
        let data = `transaction_id=${transaction_id}`;
        if (transaction_id != ''){
            sendData(data, "../controllers/admin/report_resolve.php");
        } else {
            alert("Internal Error! the transaction id was not fetched");
        }
    })
    ...
    confirm_transfer_report.addEventListener('click', () => {
        let transaction_id = document.querySelector("#transaction_id").value;
        let dept_opt = document.querySelector("#dept_id_1");
        let dept_id = dept_opt.options[dept_opt.selectedIndex].value;
        if (dept_id == 0) {
            document.querySelector('#MsgboxModalTitle').innerHTML = "Reports";
            document.querySelector('#MsgboxModalBody').innerHTML = "Please choose a department";
            $('#MsgboxModal').modal('show');
        } else {
            let data = `transaction_id=${transaction_id}&dept_id=${dept_id}`;
            sendData(data, "../controllers/admin/report_transfer.php");
        }
    })
    back_reports.addEventListener('click', () => {
